...
/**
 * Create a 16:9 LinkedIn video
 */
export async function createLinkedIn(req: Request, res: Response) {
  try {
    ...
    // Add job to queue
    const jobId = await addVideoJob('linkedin', {
      clips: clips.map(clip => {
        const baseClip = {
          image: join(data.mediaDir, clip.image),
          durationSec: clip.durationSec || 4, // Slightly longer for LinkedIn
        };
        ...
        // Default for auto-resolved clips - more conservative for LinkedIn
        return {
          ...baseClip,
          pan: 'center' as const,
          zoom: 'in' as const, // Subtle zoom in for LinkedIn content
        };
      }),
      outPath,
    ...
    logger.info({ jobId, mediaDir: data.mediaDir, outPath }, 'LinkedIn job queued');
    
    res.status(202).json({
      jobId,
      outPath,
      status: 'queued',
      estimatedDuration: clips.length * 4, // LinkedIn clips are longer
      timestamp: new Date().toISOString(),
    });
    
  } catch (error) {
    logger.error({ error, path: req.path }, 'Failed to create LinkedIn video');
    
    res.status(500).json({
      error: 'Internal Server Error',
      message: 'Failed to process LinkedIn video request',
      timestamp: new Date().toISOString(),
      path: req.path,
      statusCode: 500,
    });
...
function generateSlug(): string {
  ...
  return `linkedin_${timestamp}_${random}`;
}