...
export async function generateForCarousel({ mediaPaths, platform, brandPath, seed }) {
    ...
    const exifHints = await extractExifHints(mediaPaths[0]).catch(() => ({ lens: 'standard', timeOfDay: 'day' }));
    const promptInputs = {
        brief,
        productTags,
    ...
    // Hashtag limits
    const limitByPlatform = { instagram: 30, tiktok: 30, youtube_shorts: 15, linkedin: 4, facebook: 5 };
...
import { CaptionJSONSchema } from './types.js';
import { logger } from './logger.js';
export const MEDIA_PATTERNS = ['**/*.jpg', '**/*.jpeg', '**/*.png', '**/*.mp4'];
export async function loadBrandKit(brandPath) {
...
export async function generateForDir(options) {
    const { mediaDir, platform, outFile, providerName, model, seed, concurrency = 4, } = options;
    ...
    if (!files.length) {
        logger.warn({ mediaDir }, 'No media files found');
        return [];
    }
    ...
    // Write sidecar files
    await Promise.all(results.map((result) => writeSidecarFile(result, mediaDir)));
...
async function processMediaFile(filePath, platform, brandKit, provider, model, seed, limit) {
    const basename = path.basename(filePath, path.extname(filePath));
    // Extract EXIF hints
    const exifHints = await extractExifHints(filePath).catch(() => ({ lens: 'standard', timeOfDay: 'day' }));
    // Build prompt inputs
    const promptInputs = {
...
async function writeSidecarFile(result, mediaDir) {
