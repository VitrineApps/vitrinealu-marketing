...
try {
    while ((Get-Date) -lt $script:EndTime) {
        ...
        if ($RunningJobs.Count -gt 0) {
            ...
            Write-MainLog "Time remaining: $([math]::Round(($script:EndTime - $CurrentTime).TotalHours, 1)) hours" "INFO"
        }
...
foreach ($jobEntry in $RunningJobs.GetEnumerator()) {
    ...
    try {
        ...
        Write-MainLog "$jobName job cleaned up successfully" "SUCCESS"
...
Write-MainLog "Start Time: $($script:StartTime.ToString('yyyy-MM-dd HH:mm:ss'))" "INFO"
